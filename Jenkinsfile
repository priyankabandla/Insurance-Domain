node{
    ...
    stage('git code checkout'){
        try{
            ...
            git 'https://github.com/priyankabandla/Insurance-Domain.git'
        }
        catch(Exception e){
    ...
    stage('Containerize the application'){
       ...
       sudo  sh "${dockerCMD} build -t shubhamkushwah123/insure-me:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sudo sh "${dockerCMD} login -u shubhamkushwah123 -p ${dockerHubPassword}"
        sudo sh "${dockerCMD} push shubhamkushwah123/insure-me:${tagName}"
